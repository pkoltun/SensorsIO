...
function SensorsServer(httpServer, port, createSocketServer ) {
    var that = this;
    this.httpServer = httpServer;
    this.sensorsWithSocket = [];
    this.port = port;
    this.createSocketServer = (typeof createSocketServer === "undefined") ? function (path, logger, handleMessage) { return new WSServer(that.httpServer, path, logger, handleMessage); } : createSocketServer;
}
...
SensorsServer.prototype.addSensor = function (sensor, path, logger) {
    ...
    var wss = this.createSocketServer( path, logger, handleMessage);
    ...
    sensor.on(sensor.stateChangeEventName, function (newState) {
        ...
        wss.broadcastToAuthorized(stateChangedResponse, handleSendResult);
    });
}
